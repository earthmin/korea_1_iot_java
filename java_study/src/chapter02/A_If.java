...
//			조건이 거짓일때 실행될 코드
		...
		// cf) 실행될 코드 블록에 오는 코드가 하나의 구문이면 {} 생략가능 
...
//		각 조건식에서 저장할 문자열값(사분면의 위치)을 담을 변수 선언 
		...
		if (x > 0 && y > 0) functionScope = "1사분면";
		else if (x < 0 && y > 0) functionScope = "2사분면";
		else if (x < 0 && y < 0) functionScope = "3사분면";
		else  functionScope = "4사분면";

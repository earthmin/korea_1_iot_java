...
/*
 ...
 추상 클래스 
 : 미완성 설계도, 완전하지 않은 클래스
 >> 다른 클래스가 상속받아 완성 가능, 그 자체로는 인스턴스 생성이 불가
 
 추상 클래스의 특징 
 1. 클래스 자체로는 인스턴스 생성이 불가능 
 2. 추상 메서드를 하나 이상 포함!
 3. 확장성 제공을 위한 사용 
...
abstract class Human {
	...
	/*
	 === 추상 메서드 ===
	 : 메서드(반환타입, 메서드명, 매개변수 나열하는 소괄호)
	 : 메서드의 선언부만 존재, 구현부는 존재하지 않는 메서드(소괄호까지만있음)
	 자식 클래스에서 해당 메서드를 반드시 구현하도록 강제하는 역할 
	 >> abbstract 키워드를 사용 + 구현부 {} 대신 세미콜론 ; 을 사용하여 선언을 마침 
	 */
	abstract void work(); // 추상 메서드임 ! 구현부 없당 ~~
}
...
//		Human human = new Human(); - Error 추상 클래스를 통한 인스턴스는 불가 !
